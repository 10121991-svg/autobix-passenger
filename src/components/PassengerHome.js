...
function PassengerHome() {
  ...
  const handleRequestRide = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/passageiro/viagem', {
        method: 'POST',
  ...
  return (
    <section className="container mx-auto text-center py-20 text-white">
      ...
      <p className="text-xl mb-6">Urbiix Car: seu carro na palma da mão, sem complicação! Vem pra revolução!</p>
      <div className="flex justify-center space-x-4">
        <button onClick={handleRequestRide} className="bg-blue-500 text-white px-6 py-3 rounded hover:bg-blue-600">Quero pedir um Urbiix!</button>
